...
class Adress:
    ...
    # se o arquivo não existe, cria um arquivo novo
...
class SerialTelnetBridge:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Serial-Telnet Bridge")
        self.root.geometry("485x650")
        
        # Frame Serial
        self.connection_frame_serial = ttk.LabelFrame(root, text="Serial configuration")
        self.connection_frame_serial.pack(padx=10, pady=5, fill=tk.X)
        
        # Seleção da porta serial
        ttk.Label(self.connection_frame_serial, text="Serial Port:").grid(row=0, column=0, padx=(5,5), pady=(5,10), sticky='e')
        self.port_combo = ttk.Combobox(self.connection_frame_serial, width=10)
        self.port_combo.grid(row=0, column=1, pady=(5,10))
        ToolTip(self.port_combo, msg="COM port that will be used for serial communication.", follow=True)
        ...
        ToolTip(self.baud_combo, msg="Baud rate of the COM port for serial communication.", follow=True)
        ...
        ToolTip(self.stop_bits_combo, msg="COM port stop bits for serial communication.", follow=True)
        ...
        ToolTip(self.bits_size_combo, msg="COM port bit size for serial communication.", follow=True)
        ...
        ToolTip(self.Parity_combo, msg="COM port parity for serial communication.", follow=True)
        
        # Frame Telnet
        self.connection_frame_telnet = ttk.LabelFrame(root, text="Telnet Configuration(TCP/IP)")
        ...
        ToolTip(self.host_entry, msg="TCP host for Telnet communication.\nExample: 192.168.1.1", follow=True)
        ...
        ToolTip(self.port_entry, msg="TCP port for Telnet communication.\nExample: 23", follow=True)

        # Frame Telnet
        self.connection_frame_option = ttk.LabelFrame(root, text="Options")
        self.connection_frame_option.pack(padx=10, pady=5, fill=tk.X)

        # botão de atualizar as portas seriais
        self.refresh_btn = ttk.Button(self.connection_frame_option, text="Update Ports", command=self.refresh_ports)
        self.refresh_btn.grid(row=0, column=1, padx=10, pady=(10,0))
        ToolTip(self.refresh_btn, msg="It will update the list of existing COM ports based on the local machine.", follow=True)

        # botão de conexão
        self.connect_btn = ttk.Button(self.connection_frame_option, text="Connect", command=self.toggle_connection)
        self.connect_btn.grid(row=0, column=0, padx=10, pady=(10,0))
        ToolTip(self.connect_btn, msg="It will start the Telnet-Serial bridge.", follow=True)

        # Botão de salvar configurações
        self.save_btn = ttk.Button(self.connection_frame_option, text="Save", command=self.save_config)
        self.save_btn.grid(row=0, column=2, padx=10, pady=(10,0))
        ToolTip(self.save_btn, msg="Used to save the defined Serial | Host-Telnet | Terminal settings.", follow=True)
        ...
        ToolTip(self.hide_check, msg="Check button in case you want to hide the log terminal.", follow=True)
    ...
    def disconnect(self):
        ...
        self.connected = False
        self.data_queue.put(("warning", "Disconnecting...\n"))
        ...
        if self.serial_conn and self.serial_conn.is_open:
            try:
                ...
                self.data_queue.put(("error", f"Error closing serial: {str(e)}\n"))
        ...
        if self.telnet_writer and not self.telnet_writer.transport.is_closing():
            try:
                ...
                self.data_queue.put(("error", f"Error closing telnet: {str(e)}\n"))
        ...
        self.data_queue.put(("warning", "Disconnected. Ready for new connection.\n"))
    ...
    def connect(self):
        ...
        if not serial_port:
            self.data_queue.put(("error", "Error: No serial port selected!\n"))
            return

        # congela UI durante a tentativa de conexão
        self.set_ui_state(False)
        self.data_queue.put(("warning", f"Connecting to {serial_port}@{baud_rate} and {telnet_host}:{telnet_port}...\n"))
        ...
        try:
            ...
            self.data_queue.put(("serial", f"Connected! Reading serial: {serial_port}\n"))
            self.connect_btn.config(text="Disconnect")
            
        except Exception as e:
            self.data_queue.put(("error", f"connection error: {str(e)}\n"))
            self.set_ui_state(True)
            if self.serial_conn and self.serial_conn.is_open:
                self.serial_conn.close()
    ...
    async def telnet_worker(self, host, port):
        try:
            ...
            self.data_queue.put(("telnet", f"Connected to {host}:{port}\n"))
            ...
            self.data_queue.put(("error", f"Telnet error: {str(e)}\n"))
        finally:
            if self.connected:
    ...
    async def telnet_shell(self, reader, writer):
        while self.connected:
            try:
                ...
                self.data_queue.put(("error", f"Error in Telnet field: {str(e)}\n"))
                break
    ...
    def serial_worker(self):
        try:
            ...
            if self.connected:
                self.data_queue.put(("error", f"Error in Serial: {str(e)}\n"))
                self.root.after(100, self.disconnect)
